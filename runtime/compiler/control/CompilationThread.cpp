...
TR_YesNoMaybe TR::CompilationInfo::detectCompThreadStarvation()
{
    ...
    if (getCpuUtil()->isFunctional() && getCpuUtil()->getAvgCpuIdle() > 5 && // This is for the entire machine
...
bool TR::CompilationInfo::canProcessLowPriorityRequest()
{
    ...
    {
        if ((getCpuUtil() && getCpuUtil()->isFunctional() && getCpuUtil()->getAvgCpuIdle() > idleThreshold()
                && // enough idle time
    ...
    return (getCpuUtil() && getCpuUtil()->isFunctional() && getCpuUtil()->getAvgCpuIdle() > idleThreshold()
        && // enough idle time
...
bool TR::CompilationInfoPerThreadBase::isCPUCheapCompilation(uint32_t bcsz, TR_Hotness optLevel)
{
    ...
    if (cpuEntitlement < 150.0) // [100, 150)
    {
        ...
        if (cpuUtil->isFunctional()
            && _compInfo.getPersistentInfo()->getElapsedTime() >= TR::Options::_classLoadingPhaseInterval
            && // For first 0.5 sec we don't have valid data
            ...
            struct CompilationTrace {
                    ...
                    : _compiler(compiler)
                {
                    ...
                    if (_compiler.getOutFile() != NULL && _compiler.getOption(TR_TraceAll)) {
                        ...
                        {
                            traceMsg(&_compiler, "\n");
                            traceMsg(&_compiler, "\tclientID=%" OMR_PRIu64,
                                TR::compInfoPT->getClientData()->getClientUID());
                        }
#endif /* defined(J9VM_OPT_JITSERVER) */
                        traceMsg(&_compiler, ">\n");
                    }
