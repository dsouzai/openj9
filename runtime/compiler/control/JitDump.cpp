...
static uintptr_t traceILOfCrashedThreadProtected(struct J9PortLibrary *portLib, void *handler_arg)
{
...
static void traceILOfCrashedThread(J9VMThread *vmThread, TR::Compilation *comp, TR::FILE *jitdumpFile)
{
    ...
    uintptr_t result = 0;
    j9sig_protect(traceILOfCrashedThreadProtected, static_cast<void *>(&p), jitDumpSignalHandler, vmThread, flags,
        &result);
...
static void jitDumpRecompileWithTracing(J9VMThread *vmThread, J9Method *ramMethod, TR::CompilationInfo *compInfo,
    ...
    void *oldStartPC, TR::FILE *jitdumpFile)
{
    PORT_ACCESS_FROM_VMC(vmThread);
    J9Class *clazz;
...
omr_error_t runJitdump(char *label, J9RASdumpContext *context, J9RASdumpAgent *agent)
{
    ...
    try {
        // Process user defined options first. The user can specify any method signature to be recompiled at an
        // arbitrary dump event. This is typically used for debugging purposes so we process this option first if it
        // exists.
                        ...
                        if (NULL != bodyInfo) {
                            if (NULL == threadCompInfo) {
                                ...
                                jitDumpRecompileWithTracing(crashedThread, userMethod, compInfo, bodyInfo->getHotness(),
                                    bodyInfo->getIsProfilingBody(), NULL, bodyInfo->getIsAotedBody(),
                                    bodyInfo->getStartPCAfterPreviousCompile(), jitdumpFile);
                            } else {
                                ...
                                TR::CompilationInfoPerThreadBase::UninterruptibleOperation
                                    jitDumpForCrashedCompilationThread(*threadCompInfo);
        ...
        if (NULL == threadCompInfo) {
            ...
            if (NULL != crashedThread->gpInfo) {
                ...
                // Chances are high that the crash happen because of a miscompilation in the first JIT compiled method
            ...
            // acquire VM access (ex. to get a class signature). Other VM events, such as VM shutdown or the GC
            ...
            if (NULL != methodBeingCompiled && methodBeingCompiled->getMethodDetails().isOrdinaryMethod()) {
...

                {
                    ...
                    jitDumpRecompileWithTracing(crashedThread,
                        ...
                        comp->compileRelocatableCode(), methodBeingCompiled->_oldStartPC, jitdumpFile);
                }
            }
        }
