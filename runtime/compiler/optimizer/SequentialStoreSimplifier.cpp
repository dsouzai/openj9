    ...
    TR::Node *rootNode, NodeForwardList *combineNodeList)
{
    ...
    if (trace)
        traceMsg(comp, "Sequential Load Simplification Candidate - rootNode: %p\n", rootNode);
    ...
    if (trace)
        traceMsg(comp, "Sequential Load Simplification Candidate - rootNode: %p, byteCount: %d\n", rootNode, byteCount);

    if (!is64bitResult && (byteCount > 4)) {
        if (trace)
            traceMsg(comp, "byteCount is too high for constructing an int value. rootNode: %p, byteCount: %d\n",
                rootNode, byteCount);
        return currentTreeTop;
    }
    ...
    if ((byteCount < 2) || (byteCount > 8)) {
        if (trace)
            traceMsg(comp, "Sequential Load of size other than 2 to 8 is not supported. rootNode: %p, byteCount: %d\n",
                rootNode, byteCount);
        return currentTreeTop;
    }
    ...
    for (int i = 0; i < byteCount; i++) {
        ...
        if (trace)
            traceMsg(comp, "Sequential Load Simplification Candidate - rootNode: %p, processedByteNodes[%d]: %p\n",
                rootNode, i, processedByteNodes[i]);

        if (0 == i) {
            aloadNode = getALoadReferenceForSeqLoad(processedByteNodes[i]);
            if (trace)
                traceMsg(comp, "Sequential Load Simplification Candidate - rootNode: %p, aloadNode: %p\n", rootNode,
                    aloadNode);
            baseOffsetNode = getBaseOffsetForSeqLoad(processedByteNodes[i]);
            if (trace)
                traceMsg(comp, "Sequential Load Simplification Candidate - rootNode: %p, baseOffsetNode: %p\n",
        ...
        if (trace)
            traceMsg(comp, "Sequential Load Simplification Candidate - rootNode: %p, byteConversionNode: %p\n",
                rootNode, byteConversionNode);
        shiftValue = getShiftValueForSeqLoad(processedByteNodes[i]);
        if (trace)
            traceMsg(comp, "Sequential Load Simplification Candidate - rootNode: %p, shiftValue: %d\n", rootNode,
                shiftValue);
        ...
        if (((byteCount - 1) * 8) == shiftValue) {
            ...
            if (trace)
                traceMsg(comp, "Sequential Load Simplification Candidate - rootNode: %p, signExtendResult: %d\n",
    ...
    if (trace) {
        traceMsg(comp, "Sequential Load Simplification Candidate - rootNode: %p, byteOffsets: %ld", rootNode,
            byteOffsets[0]);
        for (int i = 1; i < byteCount; i++) {
    ...
    if (matchLittleEndianSeqLoadPattern(byteOffsets, byteCount)) {
        ...
        if (trace)
            traceMsg(comp, "byteOffsets array does not match a big or little endian pattern. rootNode: %p\n", rootNode);
        return currentTreeTop;
    }

    if (trace)
        traceMsg(comp, "Sequential Load Simplification Candidate - rootNode: %p, %s\n", rootNode,
    ...
    if (((4 == byteCount) && !swapBytes && !is64bitResult) || ((8 == byteCount) && !swapBytes)) {
        if (4 == byteCount) {
            ...
            if (trace)
                traceMsg(comp, "Recreating rootNode (%p) as iloadi.\n", rootNode);
        ...
        } else /* Handles the (8 == byteCount) case. */
        {
            ...
            if (trace)
                traceMsg(comp, "Recreating rootNode (%p) as lloadi.\n", rootNode);
        ...
        if (trace)
            traceMsg(comp, "Setting rootNode (%p) child to %p.\n", rootNode, newLoadChildNode->getFirstChild());
    ...
    } else if (8 == byteCount) /* swapBytes must be true here. */
    {
        ...
        if (trace)
            traceMsg(comp, "Recreating rootNode (%p) as lbyteswap.\n", rootNode);
        ...
        if (trace)
            traceMsg(comp, "Recreating newLoadChildNode (%p) as lloadi.\n", newLoadChildNode);
        ...
        if (is64bitResult) {
            ...
            if (signExtendResult) {
                if (trace)
                    traceMsg(comp, "Recreating rootNode (%p) as i2l.\n", rootNode);
                TR::Node::recreate(rootNode, TR::i2l);
            } else {
                if (trace)
                    traceMsg(comp, "Recreating rootNode (%p) as iu2l.\n", rootNode);
            ...
            if (!swapBytes) {
                ...
                if (trace)
                    traceMsg(comp, "Creating ibyteswapNode (%p) with child %p.\n", ibyteswapNode,
        ...
        } else /* Handles the (swapBytes && !is64bitResult) case. */
        {
            /* Inserts byteswap above the load. */
            if (trace)
                traceMsg(comp, "Recreating rootNode (%p) as ibyteswap.\n", rootNode);
        ...
        if (trace)
            traceMsg(comp, "Recreating newLoadChildNode (%p) as iloadi.\n", newLoadChildNode);
        ...
        if (signExtendResult) {
            if (is64bitResult) {
                if (trace)
                    traceMsg(comp, "Recreating rootNode (%p) as s2l.\n", rootNode);
                TR::Node::recreate(rootNode, TR::s2l);
            } else {
                if (trace)
                    traceMsg(comp, "Recreating rootNode (%p) as s2i.\n", rootNode);
                TR::Node::recreate(rootNode, TR::s2i);
            }
        } else {
            if (is64bitResult) {
                if (trace)
                    traceMsg(comp, "Recreating rootNode (%p) as su2l.\n", rootNode);
                TR::Node::recreate(rootNode, TR::su2l);
            } else {
                if (trace)
                    traceMsg(comp, "Recreating rootNode (%p) as su2i.\n", rootNode);
        ...
        if (!swapBytes) {
            ...
            if (trace)
                traceMsg(comp, "Creating sbyteswapNode (%p) with child %p.\n", sbyteswapNode,
        ...
        if (trace)
            traceMsg(comp, "Recreating newLoadChildNode (%p) as sloadi.\n", newLoadChildNode);
        ...
        if (trace)
            traceMsg(comp, "Creating mulNode (%p) with children %p and %p.\n", mulNode, mulNode->getFirstChild(),
                mulNode->getSecondChild());

        if (trace)
            traceMsg(comp, "Setting rootNode (%p) children to %p and %p.\n", rootNode, mulNode, byteConversionNodes[0]);
        ...
        if (is64bitResult) {
            if (trace)
                traceMsg(comp, "Recreating byteConversionNodes[0] (%p) as bu2l.\n", byteConversionNodes[0]);
            TR::Node::recreate(byteConversionNodes[0], TR::bu2l);
        } else {
            if (trace)
                traceMsg(comp, "Recreating byteConversionNodes[0] (%p) as bu2i.\n", byteConversionNodes[0]);
        ...
        if (signExtendResult) {
            if (is64bitResult) {
                if (trace)
                    traceMsg(comp, "Recreating newConvertChildNode (%p) as s2l.\n", newConvertChildNode);
                TR::Node::recreate(newConvertChildNode, TR::s2l);
            } else {
                if (trace)
                    traceMsg(comp, "Recreating newConvertChildNode (%p) as s2i.\n", newConvertChildNode);
                TR::Node::recreate(newConvertChildNode, TR::s2i);
            }
        } else {
            if (is64bitResult) {
                if (trace)
                    traceMsg(comp, "Recreating newConvertChildNode (%p) as su2l.\n", newConvertChildNode);
                TR::Node::recreate(newConvertChildNode, TR::su2l);
            } else {
                if (trace)
                    traceMsg(comp, "Recreating newConvertChildNode (%p) as su2i.\n", newConvertChildNode);
                TR::Node::recreate(newConvertChildNode, TR::su2i);
            }
        }
        ...
        if (swapBytes) {
            ...
            if (trace)
                traceMsg(comp, "Setting newConvertChildNode (%p) child to %p.\n", newConvertChildNode, sbyteswapNode);
        ...
        if (signExtendResult) {
            if (trace)
                traceMsg(comp, "Recreating newConvertChildNode (%p) as i2l.\n", newConvertChildNode);
            TR::Node::recreate(newConvertChildNode, TR::i2l);
        } else {
            if (trace)
                traceMsg(comp, "Recreating newConvertChildNode (%p) as iu2l.\n", newConvertChildNode);
        ...
        if (trace)
            traceMsg(comp, "Recreating shortLoadNode (%p) as sloadi.\n", shortLoadNode);
        TR::Node::recreateWithSymRef(shortLoadNode, TR::sloadi, shortSymRef);

        /* The short is never sign extended. */
        if (trace)
            traceMsg(comp, "Recreating shortConversionNode (%p) as su2l.\n", shortConversionNode);
        ...
        if (swapBytes) {
            ...
            if (trace)
                traceMsg(comp, "Setting shortConversionNode (%p) child to %p.\n", shortConversionNode, sbyteswapNode);
        ...
        if (trace)
            traceMsg(comp, "Creating andNode (%p) with children %p and %p.\n", andNode, andNode->getFirstChild(),
        ...
        if (trace)
            traceMsg(comp, "Recreating lowerIntLoadNode (%p) as iloadi.\n", lowerIntLoadNode);
        ...
        if (trace)
            traceMsg(comp, "Recreating lowerIntConversionNode (%p) as i2l.\n", lowerIntConversionNode);
        ...
        if (swapBytes) {
            ...
            if (trace)
                traceMsg(comp, "Setting lowerIntConversionNode (%p) child to %p.\n", lowerIntConversionNode,
                    ibyteswapNode1);
