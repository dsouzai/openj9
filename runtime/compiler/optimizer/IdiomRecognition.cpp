    ...
    for (n = nodesLi.getFirst(); n; n = nodesLi.getNext()) {
        if (n->isLoadVarDirect()) {
            ...
            if (!trNode->getSymbol()->isAutoOrParm()) {
                TR_ASSERT_FATAL_WITH_NODE(trNode, trNode->getSymbol()->isStatic(),
...
void TR_CISCTransformer::easyTreeSimplification(TR::Node * const node)
{
    ...
    if (node->getOpCode().isIf()) {
        ...
        if (node->getOpCodeValue() == TR::ificmplt && iconstC->getInt() == 1) {
            traceMsg(comp(), "\t\teasyTreeSimplification: Node: %p converted from ificmplt with 1 to ifcmple with 0",
                node);
                ...
                for (s = si.getFirst(); s; s = si.getNext()) {
                    if (mOpcode == s->getOpcode()) {
                        ...
                        if (result[tmpMIdx + tID] == _Embed) {
                            ...
                            if (trace())
                                traceMsg(comp(), "screening2, sameOpcode: set _NotEmbed to (%d, %d)\n", m->getID(),
                                    tID);
                        }
                    }
                }
                    ...
                    for (s = si.getFirst(); s; s = si.getNext()) {
                        if (mOpcode != s->getOpcode()) {
                            ...
                            if (result[tmpMIdx + tID] == _Embed) {
                                ...
                                result[tmpMIdx + tID] = _NotEmbed;
                                if (trace())
                                    traceMsg(comp(), "screening2, others: set _NotEmbed to (%d, %d)\n", m->getID(),
                                        tID);
                            }
                        }
                    }
    ...
    if (last && last->getOpCode().isBranch()) {
        ...
        for (auto edge = block->getSuccessors().begin(); edge != block->getSuccessors().end(); ++edge) {
            ...
            if (to != branchDestinationBlock && to != orgNextBlock) {
                if (trace())
                    traceMsg(comp(), "insertBeforeNodes added the edge (%d, %d).\n", newBlock->getNumber(),
                        to->getNumber());
...
static void traceConflictingBranches(TR_CISCTransformer *opt, TR_CISCNode *pn, List<TR_CISCNode> *matches)
{
    ...
    for (TR_CISCNode *tn = ti.getFirst(); tn != NULL; tn = ti.getNext()) {
        traceMsg(comp, "%s %d (%s)", first ? "" : ",", tn->getID(),
