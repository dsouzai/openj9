    ...
    bool currentBlockHasExceptionSuccessors, bool dontAddCalls)
{
    ...
    for (int32_t k = 0; k < callsite->numTargets(); k++) {
        ...
        if (/*calltarget->_calleeSymbol->getRecognizedMethod() == TR::com_ibm_simt_SPMDKernel_execute ||*/
            ...
            weight = 1;
            traceMsg(comp(), "Setting SIMD kernel methods weights to minimum(%d) node %p.", weight,
                callsite->_callNode);
        }
#endif
    ...
    TR::Compilation *comp)
{
    ...
    if (comp->fej9()->isChangesCurrentThread(resolvedMethod)
        ...
        if (comp->trace(OMR::inlining))
            traceMsg(comp,
                "Preventing inlining of %s as it is a JCL method annotated with @ChangesCurrentThread without its "
                "caller sharing the same annotation.\n",
    ...
    for (int32_t c = callNode->getNumChildren() - 1; c >= firstArgIndex; c--) {
        ...
        if (c == callNode->getFirstArgumentIndex() && callee && callee->convertToMethod()->isArchetypeSpecimen()
                ...
                if (valueNode && valueNode->getOpCode().hasSymbolReference()
                    ...
                    if (tracePrex)
                        traceMsg(comp,
                            ...
                            prexArg, prexArg->getKnownObjectIndex(), argument->getGlobalIndex(),
            ...
            if (argFirstChild->getOpCodeValue() == TR::loadaddr && argFirstChild->getSymbol()->isStatic()
                ...
                uintptr_t objectReferenceLocation = (uintptr_t)argFirstChild->getSymbolReference()
                                                        ->getSymbol()
                                                        ->castToStaticSymbol()
                                                        ->getStaticAddress();
