...
bool CISCTransform2FindBytes(TR_CISCTransformer *trans)
{
    ...
    if (count == -1) // single delimiter which is not constant value
    {
        ...
        if (tableNode == NULL) {
            traceMsg(comp, "Abandoning reduction: failed to create loop-invariant expression for n%un [%p]\n",
    ...
    } else if (count == 1) // single delimiter
    {
        ...
        if (comp->compileRelocatableCode()) {
            if (disptrace)
                traceMsg(comp, "Abandoning reduction since we can't relocate the static table\n");
            return false;
        }
...
bool CISCTransform2ArrayCopyC2BMixed(TR_CISCTransformer *trans)
{
    ...
    if (DISPTRACE(trans))
        traceMsg(comp, "All parameters: %x %x %x %x %x %x\n", LEloadMem, LEstoreMem, BEloadMem, BEstoreMem, flagIf,
            backIf);
...
bool CISCTransform2ArrayCmp(TR_CISCTransformer *trans)
{
    ...
    if (preHeader) {
        ...
        if (disptrace)
            traceMsg(comp,
                "searching for stores to loop indices between preheader first tree %p and first matching tree %p, "
                "looking for symrefnum %d %d\n",
    ...
    switch (ifcmp->getOpcode()) {
        ...
        case TR::iflcmpeq:
            if (constNode->getOtherInfo() != 0) {
                if (disptrace)
                    traceMsg(comp, "The exit-if is TR::if*cmpeq but the constant value is %d.\n",
        ...
        case TR::iflcmplt:
            if (constNode->getOtherInfo() != 10) {
                if (disptrace)
                    traceMsg(comp, "The exit-if is TR::if*cmplt but the constant value is %d.\n",
                        constNode->getOtherInfo());
                return false;
            }
