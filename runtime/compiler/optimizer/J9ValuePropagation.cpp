    ...
    if (isLoadFlattenableArrayElement || isStoreFlattenableArrayElement) {
        ...
        if (trace())
            traceMsg(comp(), "%s: isLoadFlattenableArrayElement %d isStoreFlattenableArrayElement %d n%dn symRef #%d\n",
                __FUNCTION__, isLoadFlattenableArrayElement, isStoreFlattenableArrayElement, node->getGlobalIndex(),
        ...
        if (trace())
            traceMsg(comp(),
                "%s: n%dn %s canTransformFlattenedArrayElementLoadStore %d "
                "canTransformUnflattenedArrayElementLoadStore %d canTransformIdentityArrayElementLoadStore %d "
                "canTransformFlattenedArrayElementLoadStoreUseTypeHint %d "
                ...
                if (secondClassChildConstraint && secondClassChildConstraint->isJavaLangClassObject() == TR_yes
                    ...
                    if (secondClassChildConstraint->getClassType()->asFixedClass()) {
                        ...
                        if (isInstanceOfResult == TR_maybe) {
                            if (trace())
                                traceMsg(comp(),
                                    "Relationship between %p and %p is unknown, quit transforming "
            ...
            if (isNonNullJavaLangClass && classChildConstraint->getClassType()
                ...
                if (comp()->fej9()->isClassArray(thisClass)) {
                    ...
                    if (!arrayComponentClass) {
                        if (trace())
                            traceMsg(comp(),
                                "Array component class cannot be remembered, quit transforming Class.getComponentType "
                                "on node %p\n",
                                node);
                        break;
                    }
        ...
        switch (rm) {
            ...
            case TR::sun_misc_Unsafe_allocateInstance: {
                ...
                if (!comp()->fej9()->isConcreteClass(newClass) || comp()->fej9()->isPrimitiveClass(newClass)
                    ...
                    if (trace())
                        traceMsg(comp(),
                            "Class is not instantiatable via bytecode new, quit transforming "
                            "Object.newInstancePrototype on node %p\n",
                            node);
                    break;
                }
                ...
                if (isNewInstProto) {
                    if (!comp()->fej9()->isClassVisible(callerClass, newClass)) {
                        if (trace())
                            traceMsg(comp(),
                                "Class is not visialbe to caller class, quit transforming Object.newInstancePrototype "
                                "on node %p\n",
                                node);
                        break;
                    }
    ...
    for (size_t idx = 0; idx < fieldCount; idx++) {
        ...
        if (trace()) {
            traceMsg(comp(), "%s %s fieldSymRef: %s fieldEntry[%d] fieldName %s fieldSig %s type %d offset %d\n",
                __FUNCTION__, comp()->getDebug()->getName(loadOpCode), comp()->getDebug()->getName(fieldSymRef),
                (int)idx, fieldEntry._fieldname, fieldEntry._typeSignature, fieldEntry._datatype.getDataType(),
                fieldEntry._offset);
        }
    ...
    TR_OpaqueClassBlock *storeClassForArrayStoreCHK, TR_OpaqueClassBlock *componentClassForArrayStoreCHK)
{
    if (trace())
        traceMsg(comp(),
            ...
            __FUNCTION__, callTree->getNode()->getGlobalIndex(), callNode->getGlobalIndex(), isLoad,
...
void J9::ValuePropagation::transformVTObjectEqNeCompare(TR_OpaqueClassBlock *containingClass, TR::Node *callNode)
{
    ...
    if (fieldCount == 0) {
        ...
        if (trace()) {
            traceMsg(comp(),
                "%s Changing n%dn from %s to %s fieldEntry[0] fieldName %s fieldSig %s type %d offset %d\n",
                __FUNCTION__, callNode->getGlobalIndex(),
                isObjectEqualityCompare ? "<objectEqualityComparison>" : "<objectInequalityComparison>",
                comp()->getDebug()->getName(cmpOpCode), fieldEntry._fieldname, fieldEntry._typeSignature,
                dataType.getDataType(), fieldEntry._offset);
            traceMsg(comp(), "    %s loadFieldSymRef %p %s \n", comp()->getDebug()->getName(loadOpCode),
                loadFieldSymRef, comp()->getDebug()->getName(loadFieldSymRef));
        }
    ...
    if (method) {
        if (!node->getOpCode().isIndirect()) {
            ...
            if (method->getRecognizedMethod() == TR::java_lang_Object_clone
                ...
                /*
                 *   n12n   acall
                 * jdk/internal/value/ValueClass.newArrayInstance(Ljdk/internal/value/CheckedType;I)[Ljava/lang/Object;
                 *   n9n      acall
                 * jdk/internal/value/NullRestrictedCheckedType.of(Ljava/lang/Class;)Ljdk/internal/value/NullRestrictedCheckedType;
                ...
                if (constraint && constraint->isFixedClass() && firstChildAcallNode->getSymbol()->isResolvedMethod()
                    && (firstChildAcallNode->getSymbol()->getResolvedMethodSymbol()->getRecognizedMethod()
                        == TR::jdk_internal_value_NullRestrictedCheckedType_of)) {
                    if (trace())
                        traceMsg(comp(),
                            "%s: node n%dn its first child fixed class %p is an instance of "
                            "NullRestrictedCheckedType\n",
                            __FUNCTION__, node->getGlobalIndex(), constraint->getClass());

                    constraint = firstChildAcallNode->getFirstChild()
                        ? getConstraint(firstChildAcallNode->getFirstChild(), isGlobal)
                        : NULL;
                    TR_OpaqueClassBlock *arrayComponentClass
                        = (constraint && constraint->isFixedClass()) ? constraint->getClass() : NULL;
                    TR_OpaqueClassBlock *nullRestrictedArrayClass = arrayComponentClass
                        ? fe()->getNullRestrictedArrayClassFromComponentClass(arrayComponentClass)
                        : NULL;
    ...
    int32_t len)
{
    ...
    if (likelySubtype) {
        ...
        if (trace()) {
            ...
            traceMsg(comp(), "%s: %.*s constraint %s: ", __FUNCTION__, length, signature,
