        ...
        if (candidate->_kind == TR::New || candidate->_kind == TR::newvalue) {
            ...
            if (candidate->_kind == TR::New) {
                ...
                if (!TR::Compiler->cls.isZeroInitializable(clazz)) {
                    logprintf(trace(), log,
                        "   Fail [%p] because the candidate is not zero initializable (that is, it has a field of a "
                        "primitive value type whose fields have not been inlined into this candidate's class)\n",
                        candidate->_node);
                    rememoize(candidate);
                    _candidates.remove(candidate);
                    continue;
