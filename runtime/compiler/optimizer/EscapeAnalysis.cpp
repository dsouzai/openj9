    ...
    for (candidate = _candidates.getFirst(); candidate; candidate = next) {
        ...
        if (candidate->_kind == TR::New || candidate->_kind == TR::newvalue) {
            ...
            if (candidate->_kind == TR::New) {
                TR_OpaqueClassBlock *clazz = (TR_OpaqueClassBlock *)candidate->_node->getFirstChild()
                                                 ->getSymbol()
                                                 ->getStaticSymbol()
                                                 ->getStaticAddress();
                ...
                if (!TR::Compiler->cls.isZeroInitializable(clazz)) {
                    if (trace())
                        traceMsg(comp(),
                            "   Fail [%p] because the candidate is not zero initializable (that is, it has a field of "
                            "a primitive value type whose fields have not been inlined into this candidate's class)\n",
                            candidate->_node);
                    rememoize(candidate);
                    _candidates.remove(candidate);
                    continue;
        ...
        if (_nodeUsesThroughAselect) {
            ...
            for (auto mi = _nodeUsesThroughAselect->begin(); mi != _nodeUsesThroughAselect->end(); mi++) {
                ...
                for (auto di = mi->second->begin(), end = mi->second->end(); di != end; di++) {
                    ...
                    traceMsg(comp(), "%s[%p] n%dn", (first ? "" : ", "), aselectNode, aselectNode->getGlobalIndex());
                    first = false;
                }
        ...
        if (usesValueNumber(candidate, valueNumber)) {
            if (!forceFail && checkIfEscapePointIsCold(candidate, reason)) {
                if (!isImmutableObject(candidate)) {
                    ...
                    if (trace())
                        traceMsg(comp(),
                            "  Marking immutable candidate [%p] as referenced in forceEscape to allow for "
                            "non-contiguous allocation, but compensating for escape at [%p]\n",
                            candidate->_node, reason);
                    candidate->setObjectIsReferenced();
                }
            } else {
            ...
            if (calledMethod) {
                ...
                if (node->getOpCodeValue() == TR::allocationFence) {
                    if (candidate->isLocalAllocation()) {
                        if (trace())
                            traceMsg(comp(),
                                "Redundant flush node [%p] found for candidate [%p]! Set omitSync flag on redundant "
                                "flush node.\n",
    ...
    if (i >= 0) {
        ...
        if (node->getOpCode().isLoadVar()) {
            ...
            if (candidate->escapesInColdBlock(_curBlock) && isImmutableObject(candidate)
                ...
                if (trace())
                    traceMsg(comp(),
                        "Preserve old node [%p] for store to non-contiguous immutable candidate [%p] that escapes in "
                        "cold block; create new tree for for direct store [%p]\n",
        ...
        if (trace())
            traceMsg(comp(), "Change node [%p] into a direct load or store of #%d (%d bytes) field %d cand %p\n", node,
                autoSymRef->getReferenceNumber(), autoSymRef->getSymbol()->getSize(), i, candidate->_node);
...
void TR_EscapeAnalysis::heapifyForColdBlocks(Candidate *candidate)
{
    ...
    for (info = coldBlockInfoIt.getFirst(); info != NULL; info = coldBlockInfoIt.getNext()) {
        ...
        if (trace())
            traceMsg(comp(),
                "heapifying %p:  isContiguousAllocation %d; _dememoizedMethodSymRef %d; new heapAllocation treetop "
                "%p\n",
                ...
                // if (postDominated)
                {
                    ...
                    if (!_blocksWithSyncs->get(nextSucc)) {
                        ...
                        // succCandidate->getNext())
                        {
                            ...
                            //    succBlockNum %d\n", succCandidate->_node, succCandidate->_index,
