...
{
    TR_ASSERT(node->getType() == TR::Address, "Can't have object info on non-address type node n%dn %p",
    ...
    if (node->getOpCode().isLoadDirect() && symbol->isAutoOrParm()) {
        ...
        if (trace()) {
            traceMsg(comp(),
                "getObjectInfoOfNode n%dn is load from auto or parm, local #%d, set node known object=%d\n",
    ...
    if (knot && node->getOpCode().isCall() && !symbol->castToMethodSymbol()->isHelper()) {
        ...
        switch (rm) {
            ...
            case TR::java_lang_invoke_DelegatingMethodHandle_getTarget: {
                ...
                return comp()->fej9()->delegatingMethodHandleTarget(comp(), dmhIndex, trace());
            }
#endif
            case TR::java_lang_invoke_Invokers_checkVarHandleGenericType: {
                ...
                if (knot && isKnownObject(adIndex) && isKnownObject(vhIndex) && !knot->isNull(vhIndex)
                    ...
                    if (trace())
                        traceMsg(comp(),
                            "Invokers_checkVarHandleGenericType with known VarHandle object %d, updating node n%dn "
                            "with known MH object %d from MH table\n",
...
void TR_MethodHandleTransformer::visitIndirectLoad(TR::TreeTop *tt, TR::Node *node)
{
    ...
    if (!symRef->isUnresolved() && symbol && (symbol->isFinal() || symbol->isArrayShadowSymbol())) {
    ...
    TR::Node *node)
{
    ...
    if (knot && isKnownObject(adIndex) && isKnownObject(vhIndex) && !knot->isNull(vhIndex) && !knot->isNull(adIndex)) {
        ...
        if (mhIndex != TR::KnownObjectTable::UNKNOWN && mhEntryIndex >= 0) {
            if (trace())
                traceMsg(comp(),
                    "Replacing Invokers.checkVarHandleGenericType call at n%dn with its resulting MH table entry. "
                    "VarHandle object %d, MH object %d\n",
            ...
            uint32_t typesAndInvokersOffset
                = comp()->fej9()->getInstanceFieldOffsetIncludingHeader("Ljava/lang/invoke/VarHandle;",
                    "typesAndInvokers", "Ljava/lang/invoke/VarHandle$TypesAndInvokers;", comp()->getCurrentMethod());
            const char *tisFieldNameAndSig
                = "java/lang/invoke/VarHandle.typesAndInvokers Ljava/lang/invoke/VarHandle$TypesAndInvokers;";
            TR::SymbolReference *tisSymRef = comp()->getSymRefTab()->findOrFabricateShadowSymbol(
                comp()->getMethodSymbol(), TR::Symbol::Java_lang_invoke_VarHandle_typesAndInvokers, TR::Address,
                typesAndInvokersOffset, false, false, false, tisFieldNameAndSig);
            ...
            TR::SymbolReference *mhTableSymRef = comp()->getSymRefTab()->findOrFabricateShadowSymbol(
                comp()->getMethodSymbol(), TR::Symbol::Java_lang_invoke_VarHandle_methodHandleTable, TR::Address,
                mhTableOffset, false, false, mhTableIsFinal, mhTableFieldNameAndSig);
            ...
            // calculate element address, transform call node into load from array, and improve MH symref with known
            // object info
