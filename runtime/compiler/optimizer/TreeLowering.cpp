    ...
    if (iterTT != blockExit) {
        ...
        while (iterTT != blockExit) {
            ...
            if (isStoreOp && iterTT->getNode()->getFirstChild() == node) {
                if (trace()) {
                    traceMsg(comp,
                        "Moving treetop containing node n%dn [%p] for store of helper call result to end of prevBlock "
                        "in preparation of final block split\n",
...
void AcmpTransformer::lower(TR::Node * const node, TR::TreeTop * const tt)
{
    ...
    if (trace())
        traceMsg(comp, "Anchoring call node under treetop n%un (0x%p)\n", anchoredCallTT->getNode()->getGlobalIndex(),
            anchoredCallTT->getNode());
    ...
    if (!sourceChild->isNonNull()) {
        ...
        if (enableTrace) {
            traceMsg(comp(), "checkValueNull n%dn is inserted before  n%dn in prevBlock %d\n",
                checkValueNull->getGlobalIndex(), checkNotNullRestrictedArray->getGlobalIndex(),
                prevBlock->getNumber());
        }
...
void LoadArrayElementTransformer::lower(TR::Node * const node, TR::TreeTop * const tt)
{
    ...
    if (enableTrace)
        traceMsg(comp,
            "Anchored call node under treetop n%un (0x%p), elementIndex under treetop n%un (0x%p), arrayBaseAddress "
            "under treetop n%un (0x%p)\n",
