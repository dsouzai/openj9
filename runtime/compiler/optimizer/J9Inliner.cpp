...
bool TR_J9VirtualCallSite::isBasicInvokeVirtual()
{
    ...
    // fprintf( stderr, "method %p, size %d, start %p, PC %p, BC: %d==%d? (%d)\n", method, methodSize, methodStart, pc,
    // bytecode, J9BCinvokevirtual, (bytecode==J9BCinvokevirtual));
            ...
            if (callSiteClass && callSiteClass != _receiverClass) {
                if (comp()->fej9()->isJavaLangObject(callSiteClass))
                    ...
                    if (comp()->trace(OMR::inlining)) {
                        ...
                        traceMsg(comp(),
                            "Receiver type %p sig %s is class of an interface method for invokevirtual, improve it to "
                            "call site receiver type %p sig %s\n",
    ...
    for (TR_ExtraAddressInfo *profiledInfo = sortedValuesIt.getFirst(); profiledInfo != NULL;
        ...
        if ((val >= minProfiledCallFrequency || (firstInstanceOfCheckFailed && val >= SECOND_BEST_MIN_CALL_FREQUENCY))
            ...
            if (callSiteClass && !isInterface() && TR::Compiler->cls.isInterfaceClass(comp(), callSiteClass)
                ...
                if (comp()->trace(OMR::inlining))
                    traceMsg(comp(),
                        "inliner: callSiteClass [%p] is an interface making it impossible to confirm correct context "
                        "of the profiled class [%p]\n",
    ...
    TR_AddressInfo *valueInfo, TR_InlinerBase *inliner)
{
    ...
    if (!callSiteClass
        ...
        if (callSiteClass && comp()->trace(OMR::inlining))
            traceMsg(comp(),
                "callSiteClass [%p] is an interface making it impossible to confirm correct context for any profiled "
                "class\n",
                callSiteClass);
        return;
    }
