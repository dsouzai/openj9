...
void J9::RecognizedCallTransformer::process_java_lang_Class_IsAssignableFrom(TR::TreeTop *treetop, TR::Node *node)
{
    ...
    node->setAndIncChild(0,
        TR::Node::createWithSymRef(TR::aloadi, 1, 1, toClass,
            comp()->getSymRefTab()->findOrCreateClassFromJavaLangClassSymbolRef()));
    node->setAndIncChild(1,
        TR::Node::createWithSymRef(TR::aloadi, 1, 1, fromClass,
            comp()->getSymRefTab()->findOrCreateClassFromJavaLangClassSymbolRef()));
    node->swapChildren();
    ...
    TR::SymbolReferenceTable::CommonNonhelperSymbol helper, bool needsNullCheck)
{
    ...
    if (isNotStaticField) {
        ...
        if (enableTrace)
            traceMsg(comp(),
                "Created node objectAdjustmentNode n%dn #%d and offsetAdjustmentNode #%d n%dn to adjust object and "
                "offset for static field\n",
        ...
        if (isNotStaticField && isObjectArrayTreeTop) {
            ...
            if (!isNotStaticField) {
                ...
                if (isObjectArrayTreeTop != NULL) {
                    ...
                    if (enableTrace)
                        traceMsg(comp(),
                            "Created array access helper block_%d that loads dataAddr pointer from array object "
                            "address\n",
                            arrayAccessBlock->getNumber());
                }
#endif /* J9VM_GC_SPARSE_HEAP_ALLOCATION */
