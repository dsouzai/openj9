...
void TR_VectorAPIExpansion::visitNodeToBuildVectorAliases(TR::Node *node, bool verifyMode)
{
    ...
    if ((opCodeValue == TR::astore || opCodeValue == TR::astorei) && !node->chkStoredValueIsIrrelevant()) {
        ...
        if (rhs->getOpCode().hasSymbolReference()) {
            ...
            if (opCodeValue == TR::astore && rhs->getOpCode().isFunctionCall()
            ...
            } else if (boxingAllowed() && opCodeValue == TR::astore && rhs->getOpCodeValue() != TR::aload) {
        ...
        if (methodElementType == TR::NoType || methodNumLanes == 0) {
            if (boxingAllowed()) {
                ...
                if (_trace)
                    traceMsg(comp(),
                        "Invalidating5 #%d (isVectorAPICall=%d) due to unknown elementType=%d or numLanes=%d in node "
                        "%p\n",
    ...
    } else if (opCode.isStoreIndirect() || node->getOpCodeValue() == TR::areturn
...
bool TR_VectorAPIExpansion::boxChild(TR::TreeTop *treeTop, TR::Node *node, uint32_t i, bool checkBoxing)
{
    ...
    if ((objectType != Vector && objectType != Mask) || // TODO: support Shuffle
        ...
        maskConv = getMaskToStoreConversion(numLanes, TR::DataType::createMaskType(elementType, vectorLength),
            maskStoreOpCode);
    ...
    if (_trace)
        traceMsg(comp(), "Boxed %s%d%s child %d of node %p into %p\n", objectType == Vector ? "Vector" : "Mask",
            bitsLength, TR::DataType::getName(elementType), i, node, newObject);
    ...
    bool checkBoxing)
{
        ...
        || parentScalarized) // TODO: support unboxing into scalars
    {
        ...
        maskConv = getLoadToMaskConversion(numLanes, TR::DataType::createMaskType(elementType, vectorLength),
            maskLoadOpCode);
    ...
    TR::Node *aladdNode
        = generateArrayElementAddressNode(comp(), payloadLoad, TR::Node::iconst(operand, 0), elementSize);
    ...
    for (TR::TreeTop *treeTop = comp()->getMethodSymbol()->getFirstTreeTop(); treeTop;
        ...
        if (_trace)
            traceMsg(comp(),
                ...
                node, opCode.getName(), vectorizedOrScalarizedNode, elementType.getDataType(), bitsLength, objectType,
                scalarized);
        ...
        if (boxingAllowed() && !vectorizedOrScalarizedNode
            && (opCodeValue == TR::astore || opCodeValue == TR::astorei || opCode.isFunctionCall()
                ...
                || opCodeValue == TR::athrow || opCodeValue == TR::awrtbar)) {
            if (_trace)
    ...
    int32_t numLanes)
{
    ...
    if (scalarSymRefs == NULL) {
        ...
        for (int32_t i = 0; i < numLanes; i++) {
            ...
            if (opt->_trace)
                traceMsg(comp, "   created new scalar symRef #%d for #%d\n", (*scalarSymRefs)[i]->getReferenceNumber(),
                    nodeSymRef->getReferenceNumber());
        }
...
TR::ILOpCodes TR_VectorAPIExpansion::getLoadToMaskConversion(int32_t numLanes, TR::DataType maskType,
...
TR::ILOpCodes TR_VectorAPIExpansion::getMaskToStoreConversion(int32_t numLanes, TR::DataType maskType,
    TR::ILOpCodes &storeOpCode)
{
    TR::ILOpCodes op;
    ...
    int32_t numChildren, vapiOpCodeType opCodeType)
{
    ...
    if (opCodeType != Blend) {
        if (!opcodeNode->getOpCode().isLoadConst()) {
            if (opt->_trace)
                traceMsg(comp, "Unknown opcode in node %p\n", node);
            return NULL;
        }
    ...
    if (mode == checkScalarization || mode == doScalarization) {
        ...
        if (mode == checkScalarization) {
            if (scalarOpCode == TR::BadILOp) {
                if (opt->_trace)
                    traceMsg(comp, "Unsupported scalar opcode in node %p\n", node);
                return NULL;
            } else {
                return node;
            }
        ...
        if (mode == checkVectorization) {
            vectorOpCode = ILOpcodeFromVectorAPIOpcode(comp, vectorAPIOpcode, resultElementType, resultVectorLength,
                objectType, opCodeType, withMask, sourceElementType, sourceVectorLength);

            if (vectorOpCode == TR::BadILOp || !isOpCodeImplemented(comp, vectorOpCode)) {
                if (opt->_trace)
                    traceMsg(comp, "Unsupported vector opcode in node %p %s\n", node,
            ...
            if (opCodeType == BroadcastInt) {
                ...
                if (!isOpCodeImplemented(comp, splatsOpCode)) {
                    if (opt->_trace)
                        traceMsg(comp, "Unsupported vsplats opcode in node %p (no codegen)\n", node);
                ...
                if (!isOpCodeImplemented(comp, splatsOpCode) || !isOpCodeImplemented(comp, subOpCode)) {
                    if (opt->_trace)
                        traceMsg(comp, "Unsupported vsplats or vsub opcode in node %p (no codegen)\n", node);
    ...
    handlerMode mode)
{
    ...
    if (!broadcastTypeNode->getOpCode().isLoadConst()) {
        if (opt->_trace)
            traceMsg(comp, "Unknown broadcast type in node %p\n", node);
        return NULL;
    }
...
TR::ILOpCodes TR_VectorAPIExpansion::ILOpcodeFromVectorAPIOpcode(TR::Compilation *comp, int32_t vectorAPIOpCode,
    ...
    bool withMask, TR::DataType sourceElementType, TR::VectorLength sourceVectorLength)
{
