...
#if !defined(PERSISTENT_COLLECTIONS_UNSUPPORTED)
void J9::Compilation::addAOTMethodDependency(TR_OpaqueClassBlock *clazz)
{
...
void J9::Compilation::addAOTMethodDependency(uintptr_t chainOffset, bool ensureClassIsInitialized)
{
    ...
    if (self()->getOptions()->getVerboseOption(TR_VerboseDependencyTrackingDetails)) {
        ...
        auto romClassOffset
            = sharedCache->startingROMClassOffsetOfClassChain(sharedCache->pointerFromOffsetInSharedCache(chainOffset));
        TR_VerboseLog::writeLineLocked(TR_Vlog_INFO,
            "Method %p dependency: chainOffset=%lu romClassOffset=%lu needsInit=%d", method, chainOffset,
            romClassOffset, ensureClassIsInitialized);
    }
}
