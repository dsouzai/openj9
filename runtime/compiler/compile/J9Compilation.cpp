...
#if defined(J9VM_OPT_JITSERVER)
   auto clientData = self()->getClientData();
   if (clientData)
      {
      bool missingLoaderInfo = false;
      const AOTCacheClassRecord *record = clientData->getClassRecord((J9Class *)clazz, self()->getStream(), missingLoaderInfo);

      if (!record)
         self()->failCompilation<J9::AOTCachePersistenceFailure>("record == NULL");
