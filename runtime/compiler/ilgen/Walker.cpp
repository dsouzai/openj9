...
int32_t TR_J9ByteCodeIlGenerator::genIfImpl(TR::ILOpCodes nodeop)
{
    ...
    if (!disableIfFolding && branchBC > _bcIndex && first->getOpCode().isLoadConst()
    ...
    uintptr_t *invokeCacheArray, bool isUnresolved)
{
    ...
    if (isUnresolved) {
        ...
        TR::SymbolReference *appendixSymRef = fej9()->refineInvokeCacheElementSymRefWithKnownObjectIndex(comp(),
            appendixNode->getSymbolReference(), invokeCacheArray);
