...
uint8_t *J9::AheadOfTimeCompile::dumpRelocationHeaderData(uint8_t *cursor, bool isVerbose)
{
    ...
    switch (kind) {
        ...
        case TR_MethodCallAddress: {
            TR_RelocationRecordMethodCallAddress *mcaRecord
                = reinterpret_cast<TR_RelocationRecordMethodCallAddress *>(reloRecord);
            traceMsg(self()->comp(), "\n Method Call Address: address=" POINTER_PRINTF_FORMAT,
                mcaRecord->address(reloTarget));
            self()->traceRelocationOffsets(startOfOffsets, offsetSize, endOfCurrentRecord, orderedPair);
        } break;
        ...
        case TR_ValidateInterfaceMethodFromCP: {
            ...
            if (isVerbose) {
                traceMsg(self()->comp(),
                    "\n Validate Interface Method From CP: methodID=%d, definingClassID=%d, beholderID=%d, "
                    "lookupID=%d, cpIndex=%d ",
        ...
        case TR_ValidateMethodFromSingleAbstractImplementer: {
            ...
            if (isVerbose) {
                traceMsg(self()->comp(),
                    "\n Validate Method From Single Abstract Implementor: methodID=%d, definingClassID=%d, "
                    "thisClassID=%d, vftSlot=%d, callerMethodID=%d ",
        ...
        case TR_HCR: {
            ...
            if (isVerbose) {
                ...
                traceMsg(self()->comp(), "%s: patch location offset = %p", recordType,
                    (void *)rwoRecord->offset(reloTarget));
            }
        } break;
        ...
        case TR_DebugCounter: {
            ...
            if (isVerbose) {
                traceMsg(self()->comp(),
                    "\n Debug Counter: Inlined site index = %d, bcIndex = %d, delta = %d, fidelity = %d, staticDelta = "
                    "%d, offsetOfNameString = %p",
        ...
        case TR_ValidateHandleMethodFromCPIndex: {
            ...
            if (isVerbose) {
                traceMsg(self()->comp(),
                    "\n Validate Handle Method From CP Index: methodID=%d, callerID=%d, cpIndex=%d, "
                    "appendixObjectNull=%s, definingClassID=%d, methodIndex=%d ",
...
void J9::AheadOfTimeCompile::dumpRelocationData()
{
    ...
    if (self()->comp()->getOption(TR_UseSymbolValidationManager)) {
        traceMsg(self()->comp(), "SCC offset of class chain offsets of well-known classes is: 0x%llx\n\n",
...
void J9::AheadOfTimeCompile::processRelocations()
{
    ...
    if (!comp->getOption(TR_DisableDependencyTracking)) {
        ...
        if (totalDependencies == 0) {
            ...
            auto dependencyChain = sharedCache->storeAOTMethodDependencies(vmThread, method, definingClass,
                dependencies.data(), dependencies.size());
            if (dependencyChain) {
                comp->getAotMethodHeaderEntry()->flags |= TR_AOTMethodHeader_TracksDependencies;
                if (comp->getOptions()->getVerboseOption(TR_VerboseDependencyTracking)) {
                    TR_VerboseLog::writeLineLocked(TR_Vlog_INFO, "Method %p compiled with %lu tracked dependencies",
