...
void J9::CodeGenerator::lowerTreeIfNeeded(TR::Node *node, int32_t childNumberOfNode, TR::Node *parent, TR::TreeTop *tt)
{
    ...
    if (node->getOpCode().isCall() && !node->getSymbol()->castToMethodSymbol()->isHelper()) {
        ...
        if (rm == TR::java_lang_invoke_MethodHandle_linkToStatic
            ...
            tempSlotStoreNode
                = TR::Node::createStore(self()->comp()->getSymRefTab()->findOrCreateVMThreadTempSlotFieldSymbolRef(),
                    numArgSlotsNode, storeOpCode);
            ...
            if (rm == TR::java_lang_invoke_MethodHandle_linkToStatic
                ...
                floatTemp1StoreNode
                    = TR::Node::createStore(self()->comp()->getSymRefTab()->findOrCreateVMThreadFloatTemp1SymbolRef(),
                        numArgsNode, storeOpCode);
        ...
        if ((rm == TR::sun_misc_Unsafe_copyMemory || rm == TR::jdk_internal_misc_Unsafe_copyMemory0)
...
#endif /* J9VM_GC_SPARSE_HEAP_ALLOCATION */
            {
                ...
                TR::TreeTop *arrayCopyTT
                    = TR::TreeTop::create(self()->comp(), arraycopyNode, tt->getNextTreeTop(), tt->getPrevTreeTop());
    ...
    if (node->getOpCode().hasSymbolReference() && (node->getOpCode().isLoad() || node->getOpCode().isStore())) {
        ...
        if (symbol->isAtLeastOrStrongerThanOpaque() && node->getDataType() == TR::Int64 && !symRef->isUnresolved()
            && self()->comp()->target().is32Bit() && !self()->getSupportsInlinedAtomicLongVolatiles()) {
            bool isLoad = false;
            TR::SymbolReference *volatileLongSymRef = NULL;
            if (node->getOpCode().isLoadVar()) {
                volatileLongSymRef = self()->comp()->getSymRefTab()->findOrCreateRuntimeHelper(TR_volatileReadLong,
                    false, false, true);
                isLoad = true;
            } else
                volatileLongSymRef = self()->comp()->getSymRefTab()->findOrCreateRuntimeHelper(TR_volatileWriteLong,
                    false, false, true);
    ...
    if (self()->comp()->getOption(TR_ReservingLocks) && node->getOpCodeValue() == TR::monent) {
        ...
        TR_PersistentClassInfo *persistentClassInfo
            = self()->comp()->getPersistentInfo()->getPersistentCHTable()->findClassInfoAfterLocking(classPointer,
                self()->comp());
    ...
    if (self()->comp()->getOptions()->enableDebugCounters() && node->getOpCode().isCall()
        ...
        if (insertMethod) {
            TR::DebugCounter::prependDebugCounter(self()->comp(),
                TR::DebugCounter::debugCounterName(self()->comp(),
                    "compilationReport.instructions:byMethod.numInvocations.(%s)", caller),
                tt);
        }
        TR::DebugCounter::prependDebugCounter(self()->comp(),
            TR::DebugCounter::debugCounterName(self()->comp(), "callers/(%s)/%d=%d", caller, bcInfo.getCallerIndex(),
...
void J9::CodeGenerator::doInstructionSelection()
{
...

    {
        ...
        if (self()->comp()->getOption(TR_SplitWarmAndColdBlocks) && !self()->comp()->compileRelocatableCode()) {
        ...
        for (TR::TreeTop *tt = self()->comp()->getStartTree(); tt;
            ...
            if (self()->comp()->getOption(TR_SplitWarmAndColdBlocks) && opCode == TR::BBEnd) {
                TR::Block *b = self()->getCurrentEvaluationBlock();
            ...
            if (self()->comp()->getOption(TR_TraceCG) || debug("traceGRA")) {
                ...
                if (tt == self()->getCurrentEvaluationTreeTop()) {
                    trfprintf(self()->comp()->getOutFile(), "------------------------------\n");
                    self()->comp()->getDebug()->dumpSingleTreeWithInstrs(tt, prevInstr->getNext(), true, true, true,
                        false);
                } else {
                    ...
                    self()->comp()->getDebug()->dumpSingleTreeWithInstrs(tt, prevInstr->getNext(), false, true, false,
                        false);
                }
                trfflush(self()->comp()->getOutFile());
            }
...
static void addValidationRecords(TR::CodeGenerator *cg)
{
    ...
    if (!classInfo->empty()) {
        for (auto info = classInfo->begin(); info != classInfo->end(); ++info) {
            ...
            if (ramMethod != cg->comp()->getCurrentMethod()->getPersistentIdentifier()) // && info->_reloKind !=
            ...
            TR_ASSERT(siteIndex < (int32_t)cg->comp()->getNumInlinedCallSites(),
...
void J9::CodeGenerator::compressedReferenceRematerialization()
{
    ...
    if (TR::Compiler->om.readBarrierType() != gc_modron_readbar_none
        || self()->comp()->getOption(TR_EnableFieldWatch)) {
        if (TR::Compiler->om.readBarrierType() != gc_modron_readbar_none)
            traceMsg(self()->comp(),
                "The compressedrefs remat opt is disabled because Concurrent Scavenger is enabled\n");
    ...
    if (self()->comp()->useCompressedPointers() && (TR::Compiler->om.compressedReferenceShift() != 0)
        && !disableRematforCP) {
        if (self()->comp()->getOption(TR_TraceCG))
    ...
    if (self()->comp()->useCompressedPointers() && !disableRematforCP) {
        for (tt = self()->comp()->getStartTree(); tt; tt = tt->getNextTreeTop()) {
            node = tt->getNode();
    ...
    TR_BitVector *globalRegsWithRegLoad, TR_BitVector &killedParms, vcount_t visitCount)
{
    ...
    if (node->getOpCode().isLoadVar() && node->getSymbol()->isParm()) {
        ...
        if (regLoads[parm->getOrdinal()] == NULL && lri != -1 && !killedParms.isSet(parm->getOrdinal())) {
            ...
            } else if (self()->comp()->target().cpu.isZ() && self()->comp()->target().isLinux()
...
void J9::CodeGenerator::addInvokeBasicCallSiteImpl(TR::Node *callNode, TR::Instruction *instr, uint8_t *retAddr)
{
    ...
    if (comp()->getOption(TR_TraceCG)) {
        ...
        traceMsg(comp(), " for n%un [%p] may target VM MethodHandle.invokeBasic\n", callNode->getGlobalIndex(),
            callNode);
    }
