...
void TR_Debug::print(TR::FILE *pOutFile, TR::X86PicDataSnippet *snippet)
{
    ...
    if (snippet->isInterface()) {
        ...
        if (methodSymRef->isUnresolved()) {
            ...
            printPrefix(pOutFile, NULL, bufferPos, sizeof(uintptr_t));
            trfprintf(pOutFile, "%s\t" POINTER_PRINTF_FORMAT "\t\t%s cpIndex", op, (void *)*(uintptr_t *)bufferPos,
                commentString());
            bufferPos += sizeof(uintptr_t);
        ...
        if (snippet->unresolvedDispatch()) {
            ...
            if (_comp->target().is64Bit()) {
                printPrefix(pOutFile, NULL, bufferPos, 1);
                trfprintf(pOutFile, "%s\t%02x\t\t\t\t\t\t\t\t%s REX of MOVRegImm64", dbString(), *bufferPos,
                    commentString());
                bufferPos += 1;
                ...
                printPrefix(pOutFile, NULL, bufferPos, 1);
                trfprintf(pOutFile, "%s\t%02x\t\t\t\t\t\t\t\t%s REX of CallMem", dbString(), *bufferPos,
                    commentString());
                bufferPos += 1;

                callModRM = *bufferPos;
                printPrefix(pOutFile, NULL, bufferPos, 1);
                trfprintf(pOutFile, "%s\t%02x\t\t\t\t\t\t\t\t%s ModRM for TR::InstOpCode::CALLMem", dbString(),
                    *bufferPos, commentString());
                bufferPos += 1;
            } else {
                printPrefix(pOutFile, NULL, bufferPos, 1);
                trfprintf(pOutFile, "%s\t%02x\t\t\t\t\t\t\t\t%s ModRM for TR::InstOpCode::CMPRegImm4", dbString(),
                    *bufferPos, commentString());
                bufferPos += 1;
            }
