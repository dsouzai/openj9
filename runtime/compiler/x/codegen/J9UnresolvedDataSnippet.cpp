...
void TR_Debug::print(TR::FILE *pOutFile, TR::UnresolvedDataSnippet *snippet)
{
    ...
    if (!snippet->getDataSymbol()->isShadow() && !snippet->getDataSymbol()->isClassObject()
        ...
        if (length < 8) {
            ...
            trfprintf(pOutFile, "%s\t(%d)\t\t\t%s patch instruction bytes + TR::InstOpCode::RET + residue", dbString(),
                bytesToCopy, commentString());
            printPrefix(pOutFile, NULL, bufferPos, 1);
            trfprintf(pOutFile, "%s\t\t\t\t\t\t%s byte that TR::InstOpCode::RET overwrote", dbString(),
                commentString());
            bufferPos++;
        } else {
            printPrefix(pOutFile, NULL, bufferPos, bytesToCopy + 1);
            bufferPos += (bytesToCopy + 1);
            trfprintf(pOutFile, "%s\t(%d)\t\t\t\t%s patch instruction bytes + TR::InstOpCode::RET", dbString(),
        ...
        if (_comp->target().is64Bit()) {
            if (snippet->getDataSymbol()->isShadow()) {
                ...
                printPrefix(pOutFile, NULL, bufferPos, bytesToCopy);
                bufferPos += bytesToCopy;
                trfprintf(pOutFile, "%s\t\t\t\t\t\t\t\t%s REX + op of TR::InstOpCode::MOV8RegImm64", dwString(),
                    commentString());
            }
        } else {
