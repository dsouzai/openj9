...
void TR_Debug::print(TR::FILE *pOutFile, TR::UnresolvedDataSnippet *snippet)
{
    ...
    printPrefix(pOutFile, NULL, bufferPos, sizeof(intptr_t));
    trfprintf(pOutFile, "DC   \t0x%p \t# Address Of Literal Pool Slot", (intptr_t)(snippet->getLiteralPoolSlot()));
    bufferPos += sizeof(intptr_t);

    // Snippet has out-of-line sequence for large offsets for instance data
    if (snippet->isInstanceData()) {
        printPrefix(pOutFile, NULL, bufferPos, sizeof(intptr_t));
        trfprintf(pOutFile, "DC   \t0x%p \t# Address to large offset slot", (intptr_t)((intptr_t *)bufferPos));
        bufferPos += sizeof(intptr_t);
