    ...
    if (!justLookForConflictingAddressNodes) {
        ...
        if (node->getOpaquePseudoRegister()) {
            ...
            if (storageRef->isTemporaryBased()) {
                ...
                if (self()->traceBCDCodeGen())
                    traceMsg(comp, "(nodeBased storageRefNode %s (%p))\n", storageRef->getNode()->getOpCode().getName(),
                        storageRef->getNode());
                ...
                if (self()->traceBCDCodeGen())
                    traceMsg(comp, "(constNodeBased storageRefNode %s (%p))\n",
    ...
    switch (dt) {
        ...
        case TR::ZonedDecimalSignLeadingEmbedded: {
            if (isPacked && digitsToClear >= 3) {
                ...
                digitsCleared = 1;
                if (self()->traceBCDCodeGen())
                    traceMsg(comp, "\t\tusing MVI for sign setting : set digitsCleared=1\n");
            } else {
                if (knownSignIs0xF) {
    ...
    if (clearSize == 0) {
        ...
        if (self()->traceBCDCodeGen())
            traceMsg(comp, "\tupdate clearSize %d to max(clearSize, regSize) = max(%d,%d) = %d\n", clearSize, clearSize,
                regSize, std::max(clearSize, regSize));
        ...
        if (reg->isInitialized() && reg->trackZeroDigits() && liveSymSize >= resultSize) {
            ...
            if (self()->traceBCDCodeGen())
                traceMsg(comp, "\tfound %d alreadyClearedBytes : adjust bytesToClear %d -> %d\n", alreadyClearedBytes,
                    bytesToClear, bytesToClear - alreadyClearedBytes);
...
void J9::Z::CodeGenerator::incRefCountForOpaquePseudoRegister(TR::Node *node)
{
    if (node->getOpaquePseudoRegister()) {
        ...
        if (ref && ref->isNodeBased() && ref->getNodeReferenceCount() > 0) {
            if (self()->traceBCDCodeGen())
                self()->comp()->getDebug()->trace("\tnode %s (%p) with storageRef #%d (%s): increment nodeRefCount "
                                                  "%d->%d when artificially incrementing ref count\n",
