...
void TR::S390J9CallSnippet::print(TR::FILE *pOutFile, TR_Debug *debug)
{
    ...
    if (getKind() == TR::Snippet::IsUnresolvedCall) {
        ...
        trfprintf(pOutFile, "LARL \tGPR14, *+%d <%p>\t# Start of Data Const.", 8 + 6 + padbytes,
        ...
        debug->printPrefix(pOutFile, NULL, bufferPos, 6);
        trfprintf(pOutFile, "BRASL \tGPR14, <%p>\t# Branch to Helper Method %s", getSnippetDestAddr(),
...
void TR_Debug::print(TR::FILE *pOutFile, TR::S390UnresolvedCallSnippet *snippet)
{
    ...
    trfprintf(pOutFile, "DC   \t0x%08x \t\t# Offset | Flag | CP Index",
        helperLookupOffset | methodSymRef->getCPIndexForVM());
}
...
void TR_Debug::print(TR::FILE *pOutFile, TR::S390VirtualUnresolvedSnippet *snippet)
{
    ...
    trfprintf(pOutFile, "DC   \t%p \t\t# Address Of Constant Pool",
        (intptr_t)getOwningMethod(methodSymRef)->constantPool());
    bufferPos += sizeof(intptr_t);
