...
void TR::ARMMonitorEnterSnippet::print(TR::FILE *pOutFile, TR_Debug *debug)
{
    ...
    debug->printPrefix(pOutFile, NULL, bufferPos, 4);
    //    beq     restartLabel
    trfprintf(pOutFile, "beq\t" POINTER_PRINTF_FORMAT "\t\t; Return to %s", (intptr_t)(restartLabel->getCodeLocation()),
...
void TR::ARMMonitorExitSnippet::print(TR::FILE *pOutFile, TR_Debug *debug)
{
    ...
    trfprintf(pOutFile, "streq\t%s, [%s, %d]; ", debug->getName(monitorReg), debug->getName(objReg),
        getLockWordOffset());
    bufferPos += 4;
