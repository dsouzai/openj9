...
bool TR_CHTable::canSkipCommit(TR::Compilation *comp)
{
    return comp->compileRelocatableCode()
        || (comp->getVirtualGuards().empty() && comp->getSideEffectGuardPatchSites()->empty() && !_preXMethods
            && !_classes && !_classesThatShouldNotBeNewlyExtended);
}
...
CHTableCommitData TR_CHTable::computeDataForCHTableCommit(TR::Compilation *comp)
{
    ...
    return std::make_tuple(classes, classesThatShouldNotBeNewlyExtended, preXMethods, sideEffectPatchSitesVec,
        ...
        classesForOSRRedefinition, classesForStaticFinalFieldModification, startPC);
}
#endif /* defined(J9VM_OPT_JITSERVER) */
