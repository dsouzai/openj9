...
TR_J9VMBase::ObjectClassInfo TR_J9VMBase::getObjectClassInfoFromObjectReferenceLocation(TR::Compilation *comp,
    ...
    TR::KnownObjectTable::Index objIndex)
{
    ...
    if (objIndex != TR::KnownObjectTable::UNKNOWN && knot && !knot->isNull(objIndex)) {
        ...
        if (vmentry == 0) {
            ...
            if (form == 0) {
                if (comp->getOption(TR_TraceOptDetails)) {
                    traceMsg(comp, "targetMethodFromMethodHandle: null ((MethodHandle)obj%d).form\n", objIndex);
                }
            ...
            if (vmentry == 0) {
                if (comp->getOption(TR_TraceOptDetails)) {
                    traceMsg(comp, "targetMethodFromMethodHandle: null ((MethodHandle)obj%d).form.vmentry\n", objIndex);
                }
...
TR::SymbolReference *TR_J9VMBase::refineInvokeCacheElementSymRefWithKnownObjectIndex(TR::Compilation *comp,
    TR::SymbolReference *originalSymRef, uintptr_t *invokeCacheArray)
{
    TR::VMAccessCriticalSection vmAccess(this);
    ...
    TR::SymbolReference *newRef
        = comp->getSymRefTab()->findOrCreateSymRefWithKnownObject(originalSymRef, arrayElementKnotIndex);
    return newRef;
}
    ...
    TR::KnownObjectTable::Index dmhIndex, bool trace)
{
    ...
    if (trace) {
        traceMsg(comp, "delegating method handle target: delegating mh obj%d(*%p) CountingWrapper %p\n", dmhIndex,
...
TR::Node *TR_J9VM::inlineNativeCall(TR::Compilation *comp, TR::TreeTop *callNodeTreeTop, TR::Node *callNode)
{
    ...
    switch (methodID) {
        ...
        case TR::java_lang_J9VMInternals_rawNewArrayInstance: {
            ...
            callNode->setSymbolReference(comp->getSymRefTab()->findOrCreateANewArraySymbolRef(
            ...
            TR::Node *newNodeChild = TR::Node::createWithSymRef(callNode, TR::aloadi, 1,
