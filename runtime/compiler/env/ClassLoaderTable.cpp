...
size_t TR_PersistentClassLoaderTable::numPermanentLoadersSentToServer(TR::CompilationInfo *compInfo) const
{
    assertSequencingMonitorHeld(compInfo);
    return _numPermanentLoadersSentToServer;
}

void TR_PersistentClassLoaderTable::addPermanentLoadersSentToServer(size_t n, TR::CompilationInfo *compInfo)
{
    assertSequencingMonitorHeld(compInfo);
    _numPermanentLoadersSentToServer += n;
}

void TR_PersistentClassLoaderTable::resetPermanentLoadersSentToServer(TR::CompilationInfo *compInfo)
{
    assertSequencingMonitorHeld(compInfo);
    _numPermanentLoadersSentToServer = 0;
}

void TR_PersistentClassLoaderTable::assertSequencingMonitorHeld(TR::CompilationInfo *compInfo) const
{
    TR_ASSERT_FATAL(compInfo->getSequencingMonitor()->owned_by_self(), "caller must hold the sequencing monitor");
}
