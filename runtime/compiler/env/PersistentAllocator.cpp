...
int PersistentAllocator::disclaimAllSegments()
{
    ...
    bool canDisclaimOnSwap = TR::Options::getCmdLineOptions()->getOption(TR_PreferSwapForMemoryDisclaim)
    ...
    for (auto segmentIterator = _segments.begin(); segmentIterator != _segments.end(); ++segmentIterator) {
        ...
        if (segment.vmemIdentifier.allocator == OMRPORT_VMEM_RESERVE_USED_MMAP_SHM || // Can disclaim to file
            ((segment.vmemIdentifier.mode & J9PORT_VMEM_MEMORY_MODE_VIRTUAL)
                && canDisclaimOnSwap)) // Can disclaim to swap
        {
            size_t segLength = segment.heapTop - segment.heapBase;
