...
TR::KnownObjectTable::Index J9::KnownObjectTable::getOrCreateIndex(uintptr_t objectPointer)
{
    ...
    if (self()->comp()->isOutOfProcessCompilation()) {
        TR_ASSERT_FATAL(false,
            "It is not safe to call getOrCreateIndex() at the server. The object pointer could have become stale at "
            "the client.");
        ...
        TR_ASSERT_FATAL(index <= nextIndex,
            "The KOT index %d at the client is greater than the KOT index %d at the server", index, nextIndex);
...
#endif /* defined(J9VM_OPT_JITSERVER) */
    {
        ...
        _references[nextIndex] = (uintptr_t *)thread->javaVM->internalVMFunctions->j9jni_createLocalRef(
