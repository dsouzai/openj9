...
bool J9::ClassEnv::isZeroInitializable(TR_OpaqueClassBlock *clazz)
{
#if defined(J9VM_OPT_JITSERVER)
    if (auto stream = TR::CompilationInfo::getStream()) {
        uintptr_t classFlags = 0;
...
j9object_t *J9::ClassEnv::getDefaultValueSlotAddress(TR::Compilation *comp, TR_OpaqueClassBlock *clazz)
{
    TR_ASSERT_FATAL(self()->isClassInitialized(comp, clazz),
        "clazz %p must be initialized when getDefaultValueSlotAddress is called", clazz);
    ...
    if (auto stream = comp->getStream()) {
        ...
        JITServerHelpers::getAndCacheRAMClassInfo((J9Class *)clazz, clientSessionData, stream,
            JITServerHelpers::CLASSINFO_DEFAULT_VALUE_SLOT_ADDRESS, (void *)&defaultValueSlotAddress);
