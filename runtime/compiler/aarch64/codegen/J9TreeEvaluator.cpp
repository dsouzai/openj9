...
void VMgenerateCatchBlockBBStartPrologue(TR::Node *node, TR::Instruction *fenceInstruction, TR::CodeGenerator *cg)
{
    ...
    if (fej9->shouldPerformEDO(block, comp)) {
        ...
        TR::Instruction *gcPoint = generateConditionalBranchInstruction(cg, TR::InstOpCode::b_cond, node, snippetLabel,
            TR::CC_EQ, conditions);
...
TR::Register *J9::ARM64::TreeEvaluator::conditionalHelperEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    TR::Instruction *gcPoint
        = generateConditionalBranchInstruction(cg, TR::InstOpCode::b_cond, node, snippetLabel, cc, conditions);
    gcPoint->ARM64NeedsGCMap(cg, 0xFFFFFFFF);
    snippet->gcMap().setGCRegisterMask(0xffffffff);
    ...
    TR::Register *addrReg, TR::Register *metaReg, TR_ARM64ScratchRegisterManager *srm, TR::LabelSymbol *callLabel)
{
    ...
    if (comp->getOption(TR_EnableMonitorCacheLookup)) {
        ...
        auto branchInstrToLookup = generateConditionalBranchInstruction(cg, TR::InstOpCode::b_cond, node,
            monitorLookupCacheLabel, TR::CC_LE);
...
TR::Register *J9::ARM64::TreeEvaluator::BNDCHKEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    gcPoint = generateConditionalBranchInstruction(cg, TR::InstOpCode::b_cond, node, snippetLabel,
        (reversed ? TR::CC_CS : TR::CC_LS));
        ...
        if (gcMode != gc_modron_wrtbar_always) {
            ...
            generateTrg1MemInstruction(cg, TR::InstOpCode::ldrimmx, node, tmp2Reg,
                TR::MemoryReference::createWithDisplacement(cg, metaReg,
                    offsetof(J9VMThread, heapSizeForBarrierRange0)));
            generateCompareInstruction(cg, node, tmp1Reg, tmp2Reg, true);
            generateConditionalBranchInstruction(cg, TR::InstOpCode::b_cond, node, doneLabel,
                TR::CC_CS); // greater or equal (unsigned)
        }
...
static TR::Register *inlineCRC32CUpdateBytes(TR::Node *node, TR::CodeGenerator *cg, bool isRawAddress, bool isCRC32C)
{
    ...
    if (isConstEndOrLen && isConstOff && !isRawAddress) {
        ...
        if (alignmentCheckNeeded) {
            ...
            auto branchToLoopHeaderInstr
                = generateConditionalBranchInstruction(cg, TR::InstOpCode::b_cond, node, loopHeaderLabel, TR::CC_EQ);
            if (debugObj) {
        ...
        auto branchToResidual4Instr
            = generateConditionalBranchInstruction(cg, TR::InstOpCode::b_cond, node, residual4Label, TR::CC_LT);
...
static TR::Register *inlineIntrinsicIndexOf(TR::Node *node, TR::CodeGenerator *cg, bool isLatin1)
{
    ...
    auto branchBackToLoopLabelInstr
        = generateConditionalBranchInstruction(cg, TR::InstOpCode::b_cond, node, loopLabel, TR::CC_EQ);
...
static TR::Register *inlineIntrinsicStringIndexOfString(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    generateTrg1MemInstruction(cg, TR::InstOpCode::ldrbimm, node, tmp1Reg,
    ...
    generateMovVectorElementToGPRInstruction(cg, TR::InstOpCode::umovxd, node, tmp1Reg, vtmp1Reg, 0);
    generateLogicalShiftLeftImmInstruction(cg, node, s1idxReg, tmp2Reg, 2,
        /* is64bit */ false); // s1idxReg is used for other purpose here
    ...
    generateTrg1MemInstruction(cg, TR::InstOpCode::vldroffq, node, vtmp1Reg,
        TR::MemoryReference::createWithIndexReg(cg, s1addrReg, s1idxReg));
    ...
    generateConditionalBranchInstruction(cg, TR::InstOpCode::b_cond, node, doneLabel,
        TR::CC_LE); // resultReg has the result

    generateTrg1MemInstruction(cg, TR::InstOpCode::ldrbimm, node, tmp1Reg,
        TR::MemoryReference::createWithIndexReg(cg, s1addrReg, s1idxReg));
    generateTrg1MemInstruction(cg, TR::InstOpCode::ldrbimm, node, tmp2Reg,
        TR::MemoryReference::createWithIndexReg(cg, s2addrReg, s2idxReg));
...
static TR::Register *inlineStringLatin1Inflate(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    auto branchToGEQ2BytesLabelInstr
        = generateConditionalBranchInstruction(cg, TR::InstOpCode::b_cond, node, GEQ2BytesLabel, TR::CC_CS);
    if (debugObj) {
    ...
    auto branchToGEQ4BytesLabelInstr
        = generateConditionalBranchInstruction(cg, TR::InstOpCode::b_cond, node, GEQ4BytesLabel, TR::CC_HI);
    if (debugObj) {
    ...
    auto branchToGEQ8BytesLabelInstr
        = generateConditionalBranchInstruction(cg, TR::InstOpCode::b_cond, node, GEQ8BytesLabel, TR::CC_HI);
    if (debugObj) {
    ...
    auto branchBackwardToLoopLabelInstr
        = generateConditionalBranchInstruction(cg, TR::InstOpCode::b_cond, node, loopLabel, TR::CC_HI);
    ...
    TR::Register *arrayLengthReg, int32_t arrayLengthVal)
{
    ...
    gcPoint
        = generateConditionalBranchInstruction(cg, TR::InstOpCode::b_cond, node, boundCheckFailSnippetLabel, TR::CC_LS);
