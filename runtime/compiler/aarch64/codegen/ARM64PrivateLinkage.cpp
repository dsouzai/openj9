...
void J9::ARM64::PrivateLinkage::createPrologue(TR::Instruction *cursor)
{
    ...
    if (!comp()->isDLT()) {
        ...
        cursor = generateConditionalBranchInstruction(cg(), TR::InstOpCode::b_cond, NULL, stackOverflowSnippetLabel,
            TR::CC_LS, cursor);
    ...
    uint32_t regMapForGC)
{
    ...
    if (enableDebugCounters) {
        ...
        if (prevCursor != cursor) {
            ...
            TR::Instruction *branchToSlot1MissedLabelInstr = generateConditionalBranchInstruction(cg,
                TR::InstOpCode::b_cond, callNode, slot1MissedLabel, TR::CC_NE, prevCursor);
    ...
    if (enableDebugCounters) {
        ...
        if (!useLastITableCache) {
            ...
            if ((prevCursor1 != cursor1) || (prevCursor2 != cursor2)) {
                ...
                TR::Instruction *branchToSlot2MissedLabelInstr = generateConditionalBranchInstruction(cg,
                    TR::InstOpCode::b_cond, callNode, slot2MissedLabel, TR::CC_NE, cursor);
            ...
            if (prevCursor1 != cursor1) {
                 ...
                 * for second cache slot. */
                TR::Instruction *cursor = generateCompareInstruction(cg, callNode, vftReg, x10Reg, true, prevCursor1);
                TR::Instruction *branchToSlot2DoneLabelInstr = generateConditionalBranchInstruction(cg,
                    TR::InstOpCode::b_cond, callNode, slot2DoneLabel, TR::CC_NE, cursor);
                /* Generating instructions after debug counter instructions. */
                TR::Instruction *slot2DoneLabelInstr
                    = generateLabelInstruction(cg, TR::InstOpCode::label, callNode, slot2DoneLabel);
                if (debugObj) {
    ...
    if (useLastITableCache && (interfaceClassOfMethod != NULL)) {
        ...
        if (enableDebugCounters) {
            ...
            if ((prevCursor1 != cursor1) || (prevCursor2 != cursor2)) {
                ...
                TR::Instruction *branchToLastITableMissedLabelInstr = generateConditionalBranchInstruction(cg,
                    TR::InstOpCode::b_cond, callNode, lastITableMissedLabel, TR::CC_NE, prevCursor1);
        ...
        gcPoint
            = generateConditionalBranchInstruction(cg, TR::InstOpCode::b_cond, callNode, ifcSnippetLabel, TR::CC_AL);
        ...
        gcPoint
            = generateConditionalBranchInstruction(cg, TR::InstOpCode::b_cond, callNode, ifcSnippetLabel, TR::CC_NE);
        TR::LabelSymbol *secondBranchAddressCacheSlotLabel = ifcSnippet->getSecondBranchAddressCacheSlotLabel();
            ...
            if (useLastITableCache && methodSymbol->isInterface()) {
                ...
                if (interfaceClassOfMethod && comp()->getPersistentInfo()->getJitState() != STARTUP_STATE) {
                    ...
                    if (numImplementers <= numPICSlots) {
                        ...
                        if (comp()->getOption(TR_TraceCG))
                            traceMsg(comp(),
                                "Found %d implementers for call to %s, can be fit into %d pic slots, disabling "
                                "lastITable cache\n",
