    ...
    TR::Register *scratchReg1, TR::Register *scratchReg2, TR::Register *scratchReg3)
{
    ...
    generateConditionalBranchInstruction(cg(), TR::InstOpCode::b_cond, callNode, releaseVMAccessSnippetLabel,
        TR::CC_NE);
    ...
    TR::Register *scratchReg0, TR::Register *zeroReg)
{
    ...
    TR::Snippet *reacquireVMAccessSnippet = new (trHeapMemory()) TR::ARM64HelperCallSnippet(cg(), callNode,
        reacquireVMAccessSnippetLabel, acqVMSymRef, reacquireVMAccessRestartLabel);
    cg()->addSnippet(reacquireVMAccessSnippet);
    generateConditionalBranchInstruction(cg(), TR::InstOpCode::b_cond, callNode, reacquireVMAccessSnippetLabel,
        TR::CC_NE);
    reacquireVMAccessSnippet->gcMap().setGCRegisterMask(0);
    ...
    switch (resolvedMethod->returnType()) {
        ...
        case TR::Int8:
            if (comp()->getSymRefTab()->isReturnTypeBool(callSymRef)) {
                ...
                generateTrg1Src1ImmInstruction(cg(), TR::InstOpCode::andimmw, callNode, returnRegister, returnRegister,
                    7);
            } else {
                // sxtb (sign extend byte)
                generateTrg1Src1ImmInstruction(cg(), TR::InstOpCode::sbfmw, callNode, returnRegister, returnRegister,
                    7);
            }
            break;
        case TR::Int16:
