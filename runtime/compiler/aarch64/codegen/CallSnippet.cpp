...
static void printArgumentFlush(TR_Debug *debug, TR::FILE *pOutFile, uint8_t *cursor, const char *mnemonic,
...
uint8_t *TR_Debug::printARM64ArgumentsFlush(TR::FILE *pOutFile, TR::Node *callNode, uint8_t *cursor, int32_t argSize)
{
    uint32_t intArgNum = 0, floatArgNum = 0, offset;
        ...
        switch (child->getDataType()) {
            ...
            case TR::Int32:
                if (!linkageProperties.getRightToLeft())
                    offset -= TR::Compiler->om.sizeofReferenceAddress();
                if (intArgNum < linkageProperties.getNumIntArgRegs()) {
                    printArgumentFlush(this, pOutFile, cursor, "strimmw", offset,
            ...
            case TR::Address:
                if (!linkageProperties.getRightToLeft())
                    offset -= TR::Compiler->om.sizeofReferenceAddress();
                if (intArgNum < linkageProperties.getNumIntArgRegs()) {
                    printArgumentFlush(this, pOutFile, cursor, "strimmx", offset,
                        machine->getRealRegister(linkageProperties.getIntegerArgumentRegister(intArgNum)), stackPtr);
                    cursor += 4;
                }
                intArgNum++;
            ...
            case TR::Float:
                if (!linkageProperties.getRightToLeft())
                    offset -= TR::Compiler->om.sizeofReferenceAddress();
                if (floatArgNum < linkageProperties.getNumFloatArgRegs()) {
                    printArgumentFlush(this, pOutFile, cursor, "vstrimms", offset,
            ...
            case TR::Double:
                if (!linkageProperties.getRightToLeft())
                    offset -= 2 * TR::Compiler->om.sizeofReferenceAddress();
                if (floatArgNum < linkageProperties.getNumFloatArgRegs()) {
                    printArgumentFlush(this, pOutFile, cursor, "vstrimmd", offset,
                        machine->getRealRegister(linkageProperties.getFloatArgumentRegister(floatArgNum)), stackPtr);
                    cursor += 4;
                }
                floatArgNum++;
...
void TR_Debug::print(TR::FILE *pOutFile, TR::ARM64CallSnippet *snippet)
{
    ...
    trfprintf(pOutFile, "%s \t" POINTER_PRINTF_FORMAT "\t\t; %s%s", (glueRef->isOSRInductionHelper() ? "b" : "bl"),
        target, getName(glueRef), info);
    bufferPos += 4;
    ...
    if (!glueRef->isOSRInductionHelper()) {
        printPrefix(pOutFile, NULL, bufferPos, sizeof(intptr_t));
        trfprintf(pOutFile, ".dword \t" POINTER_PRINTF_FORMAT "\t\t; Method Pointer",
...
void TR_Debug::print(TR::FILE *pOutFile, TR::ARM64UnresolvedCallSnippet *snippet)
{
    ...
    trfprintf(pOutFile, ".dword \t" POINTER_PRINTF_FORMAT "\t\t; Pointer To Constant Pool",
        *(reinterpret_cast<intptr_t *>(cursor)));
    cursor += sizeof(intptr_t);

    printPrefix(pOutFile, NULL, cursor, 8);
    trfprintf(pOutFile, ".dword \t%016llx\t\t; Offset | Flag | CP Index",
...
void TR_Debug::print(TR::FILE *pOutFile, TR::ARM64VirtualUnresolvedSnippet *snippet)
{
    ...
    trfprintf(pOutFile, "bl \t" POINTER_PRINTF_FORMAT "\t\t; %s", (intptr_t)cursor + distance,
...
void TR_Debug::print(TR::FILE *pOutFile, TR::ARM64InterfaceCallSnippet *snippet)
{
    TR::SymbolReference *callSymRef = snippet->getNode()->getSymbolReference();
    uint8_t *cursor = snippet->getSnippetLabel()->getCodeLocation();
