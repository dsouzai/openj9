...
void TR_Debug::print(TR::FILE *pOutFile, TR::ARM64ForceRecompilationSnippet *snippet)
{
    ...
    trfprintf(pOutFile, "bl \t0x%p\t; %s%s", (intptr_t)cursor + value,
        getName(_cg->getSymRef(TR_ARM64induceRecompilation)), info);
    cursor += ARM64_INSTRUCTION_LENGTH;
