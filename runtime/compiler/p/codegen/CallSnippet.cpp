        ...
        switch (child->getDataType()) {
            ...
            case TR::Float:
                ...
                if (floatArgNum < linkageProperties.getNumFloatArgRegs()) {
                    ...
                    print(pOutFile, stackPtr, TR_WordReg);
                    trfprintf(pOutFile, ", %d], ", offset);
                    print(pOutFile, machine->getRealRegister(linkageProperties.getFloatArgumentRegister(floatArgNum)),
                        TR_WordReg);
                    cursor += 4;
                }
                floatArgNum++;
...
void TR_Debug::print(TR::FILE *pOutFile, TR::PPCCallSnippet *snippet)
{
    ...
    if (isNativeStatic) {
        ...
        distance = *((int32_t *)cursor) & 0x03fffffc;
        distance = (distance << 6) >> 6; // sign extend
        trfprintf(pOutFile, "b \t" POINTER_PRINTF_FORMAT "\t\t;%s", (intptr_t)cursor + distance,
