...
TR::CRRuntime::setupEnvForProactiveCompilation(J9JavaVM *javaVM, J9VMThread *vmThread, TR_J9VMBase *fej9)
   ...
   if (javaVM->internalVMFunctions->isDebugOnRestoreEnabled(vmThread))
      {
      TR::Options::getCmdLineOptions()->toggleFSDOptionsForAll(false);
      TR::Options::getAOTCmdLineOptions()->toggleFSDOptionsForAll(false);
#if defined(J9VM_OPT_JITSERVER)
...
TR::CRRuntime::teardownEnvForProactiveCompilation(J9JavaVM *javaVM, J9VMThread *vmThread, TR_J9VMBase *fej9)
   ...
   if (javaVM->internalVMFunctions->isDebugOnRestoreEnabled(vmThread))
      {
      TR::Options::getCmdLineOptions()->toggleFSDOptionsForAll(true);
      TR::Options::getAOTCmdLineOptions()->toggleFSDOptionsForAll(true);
#if defined(J9VM_OPT_JITSERVER)
...
bool TR::CRRuntime::compileMethodsForCheckpoint(J9VMThread *vmThread)
   ...
   if (shouldCheckpointBeInterrupted())
      ...
      TR::CRRuntime::SetupEnvForProactiveComp setupProactiveCompEnv(this, javaVM, vmThread, fej9);
...
TR::CRRuntime::resumeJITThreadsForRestore(J9VMThread *vmThread)
   ...
   if (!_jitConfig->javaVM->internalVMFunctions->isCheckpointAllowed(vmThread))
      {
