 ...
 */
template<typename T> void TR_HashTableProfilerInfo<T>::dumpInfo(TR::FILE *logFile)
{
    uint32_t *freqs = getFrequencies();
    T *values = getKeys();
 ...
 */
template<> void TR_LinkedListProfilerInfo<TR_ByteInfo>::dumpInfo(TR::FILE *logFile);

template<typename T> void TR_LinkedListProfilerInfo<T>::dumpInfo(TR::FILE *logFile)
{
    OMR::CriticalSection lock(vpMonitor);
 ...
 * Trace properties and content.
 */
template<typename T> void TR_ArrayProfilerInfo<T>::dumpInfo(TR::FILE *logFile)
{
    OMR::CriticalSection lock(vpMonitor);
