...
static int32_t J9THREAD_PROC iprofilerThreadProc(void * entryarg)
   ...
   if (rc == JNI_OK)
      ...
      iProfiler->setIProfilerThreadLifetimeState(TR_IProfiler::IPROF_THR_ATTACHED);
      }
   else
      {
   ...
   // Now that the thread name has been set, update the lifetime state to IPROF_THR_INITIALIZED
   iProfiler->getIProfilerMonitor()->enter();
...
void TR_IProfiler::startIProfilerThread(J9JavaVM *javaVM)
   ...
   if (_iprofilerMonitor)
      ...
      if(javaVM->internalVMFunctions->createThreadWithCategory(&_iprofilerOSThread,
         ...
         // the state to IPROF_THR_ATTACHED, or failed to attach and changed the state to
...
void
TR_IProfiler::addBuffersToFreeList()
   {
   while (!_workingBufferList.isEmpty())
      {
