...
ClientSessionData::reconstructJ9SharedClassCacheDescriptorList(const std::vector<ClientSessionData::CacheDescriptor> &listOfCacheDescriptors)
   ...
   for (size_t i = 0; i < listOfCacheDescriptors.size(); i++)
      ...
      cur->cacheStartAddress = cacheDesc.cacheStartAddress;
      cur->cacheSizeBytes = cacheDesc.cacheSizeBytes;
      cur->romclassStartAddress = cacheDesc.romClassStartAddress;
      cur->metadataStartAddress = cacheDesc.metadataStartAddress;
      cur->cacheStartAddress->segmentSRP = cacheDesc.segmentSRP;
      cur->cacheStartAddress->updateSRP = cacheDesc.updateSRP;
      if (prev)
         {
         prev->next = cur;
         cur->previous = prev;
