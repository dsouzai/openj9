...
void ClientSessionData::printIProfilerCacheStats()
{
    ...
    TR_VerboseLog::writeLineLocked(TR_Vlog_JITServer,
        "Client %" OMR_PRIu64 " ProfileCache stats: methodProfileCached=%zu, bytecodes=%zu samples=%zu", getClientUID(),
        numMethodsProfiled, numBytecodeEntries, numSamples);
}
...
void ClientSessionData::addPermanentLoaders(const std::vector<J9ClassLoader *> &loaders)
{
    if (loaders.empty()) {
        return;
    }
...
void ClientSessionData::getPermanentLoaders(TR::vector<J9ClassLoader *, TR::Region &> &dest) const
{
    OMR::CriticalSection lock(_permanentLoadersMonitor);
    dest.clear();
