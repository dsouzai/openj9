...
const char *TR_RelocationRecordStaticDefaultValueInstance::name() { return "TR_StaticDefaultValueInstance"; }

TR_RelocationErrorCode TR_RelocationRecordStaticDefaultValueInstance::applyRelocation(TR_RelocationRuntime *reloRuntime,
    TR_RelocationTarget *reloTarget, uint8_t *reloLocation)
{
    uintptr_t newConstantPool = computeNewConstantPool(reloRuntime, reloTarget, constantPool(reloTarget));
    TR_OpaqueClassBlock *newClassAddress
        = computeNewClassAddress(reloRuntime, newConstantPool, inlinedSiteIndex(reloTarget), cpIndex(reloTarget));

    if (!newClassAddress || !reloRuntime->comp()->fej9()->isClassInitialized(newClassAddress))
        return TR_RelocationErrorCode::staticDefaultValueInstanceRelocationFailure;

    j9object_t *newDefaultValueSlotAddress
        = TR::Compiler->cls.getDefaultValueSlotAddress(reloRuntime->comp(), newClassAddress);
    ...
    TR_RelocationTarget *reloTarget, uint8_t *reloLocationHigh, uint8_t *reloLocationLow)
{
    ...
    reloTarget->storeAddress((uint8_t *)newDefaultValueSlotAddress, reloLocationHigh, reloLocationLow,
        reloFlags(reloTarget));
...
uint32_t TR_RelocationRecord::_relocationRecordHeaderSizeTable[TR_NumExternalRelocationKinds] = {
    ...
    sizeof(TR_RelocationRecordConstantPoolWithIndexBinaryTemplate), // TR_StaticDefaultValueInstance                   =
                                                                    // 111
    ...
    sizeof(
        TR_RelocationRecordValidateDynamicMethodFromCallsiteIndexBinaryTemplate), // TR_ValidateDynamicMethodFromCallsiteIndex
                                                                                  // = 116
    sizeof(
        TR_RelocationRecordValidateHandleMethodFromCPIndexBinaryTemplate), // TR_ValidateHandleMethodFromCPIndex = 117
